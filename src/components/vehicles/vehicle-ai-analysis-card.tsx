...
export function VehicleAiAnalysisCard({
  ...
  const marketDeltaPct =
    ((vehicle.price.amount - vehicle.estimatedMarketValue.amount) / vehicle.estimatedMarketValue.amount) * 100;
  ...
  const bullets: string[] = [
    ...
    // Cost figures need profit:read; skip this bullet rather than show a fabricated number when they are not visible.
    ...(totalCost !== null && expectedProfit !== null && profitMarginPct !== null
      ? [
          ...
          )}, projected gross profit is ${formatMoney({ amount: expectedProfit, currency: vehicle.price.currency })} (${profitMarginPct.toFixed(
            1
          )}% margin).`,
        ]
      : []),
