...
export default function EditVehiclePage({ params }: { params: Promise<{ vehicleId: string }> }) {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      horsepower: vehicle.spec.horsepower,
      fuelType: vehicle.spec.fuelType,
      transmission: vehicle.spec.transmission,
      exteriorColor: vehicle.spec.exteriorColor,
      interiorColor: vehicle.spec.interiorColor,
      seats: vehicle.spec.seats,
      bodyType: vehicle.spec.bodyType,
      vin: vehicle.spec.vin,
      importSpec: vehicle.spec.importSpec,
      accidentHistory: vehicle.spec.accidentHistory,
      serviceHistory: vehicle.spec.serviceHistory,
      owners: vehicle.spec.owners,
      location: vehicle.location,
      emirate: vehicle.emirate,
      sourceType: vehicle.sourceType,
      registrationStatus: vehicle.registration.status,
      plateNumber: vehicle.registration.plateNumber,
      registrationExpiryDate: vehicle.registration.expiryDate,
      rtaNotes: vehicle.registration.rtaNotes,
      notes: vehicle.notes,
    });
  }, [vehicle, reset]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      toast.success(`${updated.year} ${updated.make} ${updated.model} updated`);
  ...
  return (
    <>
      <div className="flex items-center gap-3">
        ...
        <PageHeader title={`${t("inventory.actions.edit")} — ${vehicle.year} ${vehicle.make} ${vehicle.model}`} />
      </div>
      ...
      <form onSubmit={handleSubmit((values) => mutation.mutate(values))} className="flex flex-col gap-6">
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" asChild>
            <Link href={`/inventory/${vehicleId}`}>{t("common.cancel")}</Link>
          </Button>
