import type { Currency, ID, PaginatedResult } from "@/types/common";
import type {
  AccidentHistory,
  FuelType,
  ImportSpec,
 ...
 * on Vehicle in types/vehicle.ts. `images` is always `[]` live: photos live in Storage (`/vehicles/:id/photos`,
 * §0.9), not on this endpoint; VehicleCard/VehicleGallery already show a placeholder for an empty list.
 */
...
function toVehicle(d: VehicleDto): Vehicle {
  ...
  return {
    ...
    // Photos live in Storage (/vehicles/:id/photos, §0.9), not on this endpoint. Every gallery/card handles [].
    images: [],
