...
| `scripts/check-storage-unit.ts`, `check-storage-policies.ts`, `check-storage-http.ts` | 79 + 24 + 114 assertions (`npm run check:storage`, `check:storage-policies`, `check:storage-http`) |
...
| 17 | Vehicle images upload correctly | **The backend is real and tested; the UI does not call it yet.** `/vehicles/:id/photos/*` (signed upload, complete, list, delete) exist and pass `check:storage`, `check:storage-policies`, `check:storage-http` (79+24+114) against a fake Supabase Storage — not a real one. But `VehiclePhotoUploader` (the only photo-picker in the UI, on the AI Marketing page) only creates a local `URL.createObjectURL` preview; it never calls the upload endpoints. Nothing in the app today actually uploads a vehicle photo to Storage. |
...
- Vehicle photo and document upload have a real, tested backend and no UI wired to it (items 17, 18) — the single biggest gap this pass surfaces.
